...
public class SwipeListViewTouchListener implements View.OnTouchListener {
	...
	private String TAG = "swipeLibrary";
	...
	private void generateAnimate(final View view, final boolean swap,
		...
		Log.d("SwipeListView", "swap: " + swap + " - swapRight: " + swapRight
				+ " - position: " + position + "  " + swipeCurrentAction);
	...
	private void generateChoiceAnimate(final View view, final int position) {
		Log.d(TAG, "generateChoiceAnimate() called");
		animate(view).translationX(0).setDuration(animationTime)
				.setListener(new AnimatorListenerAdapter() {
					...
					public void onAnimationEnd(Animator animation) {
						...
						Log.d("SwipeListView",
								"resetCell from generateChoiceAnimate ");
						resetCell();
					}
				});
	}
	...
	private void generateDismissAnimate(final View view, final boolean swap,
			final boolean swapRight, final int position) {
		Log.d(TAG, "generateDismissAnimate() called");
		int moveTo = 0;
		if (opened.get(position)) {
			if (!swap) {
		...
		animate(view).translationX(moveTo).alpha(alpha)
				...
				.setListener(new AnimatorListenerAdapter() {
					...
					public void onAnimationEnd(Animator animation) {
						...
						Log.d("SwipeListView",
								"resetCell from generateDismissAnimate ");
						resetCell();
					}
				});
	...
	private void generateRevealAnimate(final View view, final boolean swap,
			final boolean swapRight, final int position) {
		// Commented by Preethika
		Log.d("Priti", "generateRevealAnimate");
		int moveTo = 0;
		if (opened.get(position)) {
			if (!swap) {
				moveTo = openedRight.get(position) ? (int) (viewWidth - rightOffset)
		...
		animate(view).translationX(moveTo).setDuration(animationTime)
				.setListener(new AnimatorListenerAdapter() {
					...
					public void onAnimationEnd(Animator animation) {
						...
						Log.d("SwipeListView",
								"resetCell from generateRevealAnimate ");
						resetCell();
					}
				});
	}
	...
	private void resetCell() {
		...
		if (downPosition != ListView.INVALID_POSITION) {
			...
			frontView = null;
			backView = null;
			Log.d("swipeLibrary", "downPosition= ListView.INVALID_POSITION;");
		...
		return new AbsListView.OnScrollListener() {
			...
			public void onScrollStateChanged(AbsListView absListView,
				...
				if (scrollState != AbsListView.OnScrollListener.SCROLL_STATE_FLING
						&& scrollState != SCROLL_STATE_TOUCH_SCROLL) {
					Log.d("swipeLibrary", "downPosition");
	...
	void closeOpenedItems() {
		Log.d(TAG, "closeOpenedItems() called");
		if (opened != null) {
			int start = swipeListView.getFirstVisiblePosition();
			int end = swipeListView.getLastVisiblePosition();
	...
	public boolean onTouch(View view, MotionEvent motionEvent) {
		...
		case MotionEvent.ACTION_DOWN: {
			Log.d("swipeLibrary", "ACTION_DOWN ");
			...
			for (int i = 0; i < childCount; i++) {
				...
				if (allowSwipe && rect.contains(x, y)) {
					...
					Log.d("swipeLibrary",
							"velocity "
									+ Math.abs(velocityTracker.getXVelocity()));
					if (swipeBackView > 0) {
		...
		case MotionEvent.ACTION_UP: {
			Log.d("swipeLibrary", "ACTION_UP ");

			// added by Amit to get the current row being swapped

			int currentTouchedPosition = -9999;
			View currentTouchedView = null;
			int childCount = swipeListView.getChildCount();
			int[] listViewCoords = new int[2];
			swipeListView.getLocationOnScreen(listViewCoords);
			int x = (int) motionEvent.getRawX() - listViewCoords[0];
			...
			for (int i = 0; i < childCount; i++) {
				...
				if (rect.contains(x, y)) {
					...
					currentTouchedPosition = childPosition;
					currentTouchedView = child.findViewById(swipeFrontView);
				}

			}

			Log.d(TAG, "down position = " + downPosition);
			if (velocityTracker == null || !swiping
					|| downPosition == ListView.INVALID_POSITION) {

				if (currentTouchedPosition != -9999
					...
					if (!opened.get(currentTouchedPosition)) {
					...
					if (minFlingVelocity <= velocityX && velocityX <= maxFlingVelocity
						...
						if (swapRight != swipingRight
							...
							swap = false;
						} else if (opened.get(currentTouchedPosition)
								&& openedRight.get(currentTouchedPosition) && swapRight) {
							swap = false;
						} else if (opened.get(currentTouchedPosition)
								&& !openedRight.get(currentTouchedPosition) && !swapRight) {
							swap = false;
					...
					generateAnimate(currentTouchedView, swap, swapRight, currentTouchedPosition);

					if (swipeCurrentAction == SwipeListView.SWIPE_ACTION_CHOICE) {
						swapChoiceState(currentTouchedPosition);
					}

					Log.d("swipeLibrary", "velocity x " + velocityX + " y " + velocityY);
					...
					//generateRevealAnimate(currentTouchedView, mySwap, mySwapRight,
							//currentTouchedPosition);
					
					/*if (swipeCurrentAction == SwipeListView.SWIPE_ACTION_CHOICE) {
						Log.d(TAG, "swipeCurrentAction == SwipeListView.SWIPE_ACTION_CHOICE");
						swapChoiceState(currentTouchedPosition);
					}*/
		...
		case MotionEvent.ACTION_MOVE: {
			Log.d("swipeLibrary", "ACTION_MOVE ");
			...
			Log.d("swipeLibrary", "ACTION_MOVE 1 ");
			...
			Log.d("swipeLibrary", "ACTION_MOVE 2");
			if (deltaMode > slop
					&& swipeCurrentAction == SwipeListView.SWIPE_ACTION_NONE
					&& velocityY < velocityX) {
				swiping = true;
			...
			Log.d("swipeLibrary", "ACTION_MOVE 3");
			Log.d("swipeLibrary",
			...
			if (swiping && downPosition != ListView.INVALID_POSITION) {
				...
				Log.d("swipeLibrary", "deltaX " + deltaX);
				move(deltaX);
				return true;
			}
			break;
	...
	public void move(float deltaX) {
		...
		if (posX > 0 && !swipingRight) {
			Log.d("swipeLibrary", "change to right ");
			/*
			 * Commented by Preethika Log.d("SwipeListView", "change to right");
			 * swipingRight = !swipingRight; swipeCurrentAction =
			 * swipeActionRight; if (swipeCurrentAction ==
			 * SwipeListView.SWIPE_ACTION_CHOICE) {
			 * backView.setVisibility(View.GONE); } else {
			 * backView.setVisibility(View.VISIBLE); }
			 */
		}
		if (posX < 0 && swipingRight) {
		...
		if (swipeCurrentAction == SwipeListView.SWIPE_ACTION_DISMISS) {
			...
			Log.d("SwipeListView", "SWIPE_ACTION_DISMISS");
			...
			if ((swipingRight && deltaX > 0 && posX < DISPLACE_CHOICE)
				...
				Log.d("SwipeListView", "SWIPE_ACTION_CHOICE if");
				setTranslationX(frontView, deltaX);
			}
		} else {
			Log.d("SwipeListView", "SWIPE_ACTION_CHOICE if else");
			setTranslationX(frontView, deltaX);
		}
	}
	...
	protected void performDismiss(final View dismissView,
		...
		Log.d(TAG, "performDismiss() called. Dismiss Position = "
				+ dismissPosition);
